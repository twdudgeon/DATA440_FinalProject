...
if uploaded_file:
    ...
    try:
        ...
        # replace import data_clean3 as data_cleaner
        from test_clean import AutoCategorizer

        # inside your file upload conditional after reading `df`:
        categorizer = AutoCategorizer(unique_threshold=20)
        cleaned_df, category_df, data_dict = categorizer.process(df.copy())

        # category_df is a DataFrame you can show in Streamlit
        st.dataframe(category_df)

        # store data_dict in session state if you want to re-use metadata
